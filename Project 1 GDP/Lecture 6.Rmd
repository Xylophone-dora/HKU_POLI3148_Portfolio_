...
#Dora: Artificial manipulated dataset to show the possible terrible outcome using `bind_rows` thoughtlessly.
```
...
#Dora: Remember to check the row names manually
```
...
-   Do `bind_rows` and `bind_cols` ONLY WHEN you know for sure that there will not be a mismatch!(Dora: Especially for `bind_rows` as the dataset may be sorted differently!)
...
```{r}
#Dora: Identifier Key means: the name of a column
...
```{r}
#Dora: Check the left to see if they are in the right, and keep the TRUE.
...
```{r}
#Dora: Check the left to see if they are in the right, and keep the FALSE.
...
#Dora: 1. Non-existing records? 2. Record exists but no data (thus as NA)? This can help us distinguish between the two.
```
...
```{r}
# Save to a .csv file
write_csv(d_gdp_1945_2022, "gdp_1945_2002.csv")

# Save to a .rds file, Dora: usually smaller! (Useful with large dataset)
saveRDS(d_gdp_1945_2022, "gdp_1945_2002.rds")
```
...
```{r}
# Read a .csv file
d_read_1 <- read_csv("gdp_1945_2002.csv")

# Read a .rds file
d_read_2 <- readRDS("gdp_1945_2002.rds")
```
